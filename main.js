...
function setup() {
	...
	label_size = createSpan('Kích cỡ cây');
	label_size.position(150, 10);
	label_level = createSpan('Độ nở hoa');
	label_level.position(150, 30);
	label_rot = createSpan('Hình dạng');
	label_rot.position(150, 50);
	label_lenRand = createSpan('Chiều dài cây');
	label_lenRand.position(150, 70);
	label_branchProb = createSpan('Số lượng nhánh cây');
	label_branchProb.position(150, 90);
	label_rotRand = createSpan('Góc nghiêng');
	label_rotRand.position(150, 110);
	label_leafProb = createSpan('Số lượng hoa');
	label_leafProb.position(150, 130);

	label_seed = createSpan('Cây Random:');
	label_seed.position(10, 162);

	input_seed = createInput(randSeed);
	input_seed.position(90, 160);
	input_seed.style('width', '50px')

	button_seed = createButton('Coi cây lớn!');
	button_seed.position(150, 160);
	button_seed.mousePressed(function () {
		randSeed = input_seed.value();
		startGrow();
	});

	button_newSeed = createButton('Tạo cây mới');
	...
	button_change = createButton('Chuyển động');
	button_change.position(10, 220);
	button_change.mousePressed(function () {
		if (!mutating) {
			button_change.html('Ngừng chuyển động')
			mutateTime = millis();
			mutating = true;
			mutate();
		}
	...
	button_hide = createButton('Ẩn Settings (click thêm lần nữa để hiện lại)');
	button_hide.position(10, 250);
	button_hide.mousePressed(hideUI);

	label_perf = createSpan('Generated in #ms');
	label_perf.position(10, 280);
	//label_perf.style('display', 'none');

	label_source = createSpan('');
	label_source.position(10, 330);

	label_source2 = createSpan('');
